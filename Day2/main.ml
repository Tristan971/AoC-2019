...
type noun_verb = { noun : int; verb : int }
...
let exec_for (input : string) =
  let program = IntcodeComputer.intcode_program_of input "" in
  let result = IntcodeComputer.execute program in
  Array.to_list result.memory
...
let exec_with (input : string) (params : noun_verb) : int array =
  ...
  input_arr.(1) <- params.noun;
  input_arr.(2) <- params.verb;
  let result = IntcodeComputer.execute { memory = input_arr; inputs = [] } in
  result.memory

let read_input () : string =
  let input = IOUtils.read_all_lines "./Day2/input" in
  match input with h :: _ -> h | _ -> failwith "Invalid input given!"
...
let part1 () : unit =
  ...
  let params : noun_verb = { noun = 12; verb = 2 } in
  ...
  let next (params : noun_verb) : noun_verb =
    let next_params =
      if params.noun < 99 then { noun = params.noun + 1; verb = params.verb }
        ...
        failwith
          ( "No next after noun at " ^ string_of_int params.noun
          ^ " and verb at " ^ string_of_int params.verb )
    in
    (*Printf.printf "-> Next params: { %d, %d }\n" next_params.noun next_params.verb;*)
    next_params
  in

  let rec search_em (params : noun_verb) =
    let result = exec_with input params in
    let output = result.(0) in
    match output with 19690720 -> params | _ -> search_em (next params)
  in
