...
let exec_with (program : string) (inputs : string) : int array =
  let result =
    IntcodeComputer.execute (IntcodeComputer.intcode_program_of program inputs)
  in
  result.memory
...
let part1 () =
  (* input 1 *)
  let input = IOUtils.read_all_lines "Day5/input" in
  exec_with (List.hd input) "1"

let part2 () =
  (* input 5 *)
  let input = IOUtils.read_all_lines "Day5/input" in
  exec_with (List.hd input) "5"
